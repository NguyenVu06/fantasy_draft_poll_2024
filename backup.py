...
def main():
    ...
    options = ["None","Nick (commish)", "Nguyen (3x champ)", "MyLinh", "Tuan", "David", "Andrew", "Joel", "Minh", "Dima", "Dan", "Anthony", "Cliffton"]
    ...
    valid_date_ranges = [
        (pd.to_datetime("today").date(), datetime.date(2025, 9, 3).date()),
    ...
    if not any(start <= selected_date <= end for start, end in valid_date_ranges):
        st.error("Please select a date within the valid ranges (Today to Sep 3).")
    else:
